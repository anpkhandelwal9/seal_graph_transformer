...
from seal_graph_transformer.utils.collator import collator
...
np.random.seed(time_int)

torch.manual_seed(time_int)
...
class SEALDynamicDataset(Dataset):
    def __init__(self, root, data, split_edge, num_hops, percent=100, split='train', 
        ...
        self.degrees = torch.tensor(self.A.sum(axis=1)).view(-1)
        if self.directed:
...
def train(train_dataset, train_loader, model, optimizer, device):
    model.train()
    ...
    for data in pbar:
        ...
        optimizer.zero_grad()
        logits = model(data)
        y = torch.tensor([x.y for x in data]).to(device)
...
@torch.no_grad()
def test(model, val_loader, val_dataset, test_loader, test_dataset, device):
    model.eval()
    total_loss=0
...
def run(args, run_no):
    ...
    if args.save_appendix == '':
        args.save_appendix = '_' + args.model + '_' + str(args.lr) + '_' + '_' + str(args.embedding_dim) + "_" + str(args.num_hops) + '_' + time.strftime("%Y%m%d%H%M%S")
    ...
    args.res_dir = os.path.join('script_results/{}{}'.format(args.dataset, args.save_appendix))
    print('Results will be saved in ' + args.res_dir)
    if not os.path.exists(args.res_dir):
        os.makedirs(args.res_dir) 
    if not args.keep_old:
        # Backup python files.
        copy('graphormer_train_multigpu1.py', args.res_dir)
        copy('./seal_graph_transformer/modules/tokengt_graph_encoder.py', args.res_dir)
        copy('./seal_graph_transformer/self_graph_transformer.py', args.res_dir)
    ...
    # Copied from RandomSampler Code
    generator = torch.Generator()
    generator.manual_seed(int(time.time()))
    train_loader = DataListLoader(train_dataset, batch_size=args.batch_size, #collate_fn=collator,
                            shuffle=True, generator=generator, num_workers=args.num_workers)
    val_loader = DataListLoader(val_dataset, batch_size=args.batch_size,  #collate_fn=collator,#shuffle=True,
                        num_workers=args.num_workers)
    test_loader = DataListLoader(test_dataset, batch_size=args.batch_size,  #collate_fn=collator,#shuffle=True,
                            num_workers=args.num_workers)
    ...
    for run in range(args.runs):
        ...
        elif args.model == 'SealGT':
            model = TokenGTModel(embedding_dim=args.embedding_dim, num_classes=1, num_node_features=args.num_node_features, num_edge_features=1,
             max_nodes=512, positional_embedding=args.positional_embedding, performer=True).to(device)
            #Cora 1433 Citeseer 3703

        if args.device == 'multi-gpu':
        ...
        with open(log_file, 'a') as f:
            ...
            print(f'Model is {args.model}', file=f)
            print(f'Dataset is {args.dataset}', file=f)
            print(f'Num_edge_feature is 1', file=f)
            if args.model == 'DGCNN':
        ...
        if args.only_test:
            results, loss_results = test(model, val_loader, val_dataset,
                                             test_loader, test_dataset, device)
            for key, result in results.items():
                loggers[key].add_result(run, result)
            for key, result in results.items():
                valid_res, test_res = result
                print(key)
                print(f'Run: {run_no + 1:02d}, '
        ...
        for epoch in range(start_epoch, start_epoch + args.epochs):
            loss = train(train_dataset, train_loader, model, optimizer, device)
                ...
                if epoch % args.log_steps == 0:
                    ...
                    for key, result in results.items():
                        ...
                        to_print = (f'Run: {run + 1:02d}, Epoch: {epoch:02d}, ' +
                                    f'Loss: {loss:.4f}, Valid: {100 * valid_res:.2f}%, Valid Loss: {val_loss:.4f}, ' +
                                    f'Test: {100 * test_res:.2f}%, Test Loss: {test_loss:.4f} ')
                        print(key)
                        print(to_print)
                        with open(log_file, 'a') as f:
                            print(key, file=f)
...
parser.add_argument('--model', type=str, default='SealGT')
parser.add_argument('--sortpool_k', type=float, default=0.6)
parser.add_argument('--num_layers', type=int, default=3)
parser.add_argument('--num_node_features', type=int, default=32)
...
parser.add_argument('--z_embedding', action='store_false')
parser.add_argument('--dist_embedding', action='store_false')
parser.add_argument('--degree_embedding', action='store_false')
...
lrs = [1e-4, 1e-5]
num_hops = [2, 1] #1
num_nodes_per_hop = [50, None] #None
num_runs = 2
