...
from seal_graph_transformer.self_graph_transformer import CustomData
...
def graph_tokenizer(adj, edge_index, src, dst, max_token_len=512):
    ...
    #get eigenvalue from adj
    # indegree = get_indegree(adj)
    # eigvec, eigval = lap_eig(adj, z.size(), indegree)

    #make edge list from Adj
    #edge_index, _ = torch_geometric.utils.convert.from_scipy_sparse_matrix(adj)
    ...
    return z.to(torch.long), dist_data.to(torch.long), #torch.Tensor(mask).to(torch.long)
...
def construct_pyg_graph(node_ids, adj, dists, node_features, y, degrees, node_label='drnl'):
    ...
    if node_label.startswith('drnl'):  # DRNL
        ...
        z, dist = graph_tokenizer(adj, edge_index, 0, 1)  #return tokens (size : number of edges in subgraph)
