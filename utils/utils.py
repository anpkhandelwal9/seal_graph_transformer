...
def graph_tokenizer(adj, edge_index, src, dst, max_token_len=512):
    ...
    edge_data = [[999 if np.isinf(dist2src[edge_index[0][i]]) else dist2src[edge_index[0][i]],
                  999 if np.isinf(dist2dst[edge_index[1][i]]) else dist2dst[edge_index[1][i]]]
    ...
    # make edge list from Adj
    # #make input ids with zero padding
    # token = [0 for _ in range(max_token_len)]
    # token[0] = 101.

    # #make mask with zero padding
    # mask = [0 for _ in range(max_token_len)]
    # mask[0] = 1
    # cur_idx = 1
    # exists = {}
    # for i in range(0, min(len(edge_index[0]), max_token_len-1)):
    #     if edge_index[0][i] == src and edge_index[1][i] == dst:
    #         continue

    #     if edge_index[0][i] == dst and edge_index[1][i] == src:
    #         continue

    #     if z[edge_index[0][i]] == 0 and  z[edge_index[1][i]] == 0:
    #         continue
    ...
    #     # if (exists.get(int(val.item()), 0) == 1):
    #     #     continue

    #     token[cur_idx] = val
    #     mask[cur_idx] = 1
    #     # exists[int(val.item())] = 1
    #     cur_idx+=1
    ...
    dist2src = [-1 if np.isinf(x) else x for x in dist2src]
    dist2dst = [-1 if np.isinf(x) else x for x in dist2dst]
    node_dist = [[dist2src[i], dist2dst[i]] for i in range(len(dist2src))]
    ...
    return z.to(torch.long), edge_data.to(torch.long), in_degree, None, None, torch.tensor(node_dist).to(torch.long)
...
def construct_pyg_graph(node_ids, adj, dists, node_features, y, node_label='drnl'):
    ...
    if node_label.startswith('drnl'):  # DRNL
        ...
        z, edge_data, in_degree, lap_eigval, lap_eigvec, node_dist = graph_tokenizer(
            adj, edge_index, 0, 1)  # return tokens (size : number of edges in subgraph)
        # edge_weight.unsqueeze(1).to(torch.long)
        data = CustomData(node_data=node_features.to(torch.float), edge_index=edge_index, edge_data=edge_weight.unsqueeze(1).to(torch.long), y=y, z_data=z,
                          edge_num=torch.tensor([edge_index.shape[1]]).to(torch.long), node_num=torch.tensor([num_nodes]).to(torch.long),
                          # lap_eigval=lap_eigval, lap_eigvec=lap_eigvec, in_degree=in_degree)
                          in_degree=in_degree, node_dist=node_dist)
        return data
    else:
