...
# def k_hop_subgraph_djikstra(src, dst, num_hops, A, sample_ratio=1.0,
#                             max_nodes_per_hop=None, node_features=None,
#                             y=1, directed=False, A_csc=None, max_nodes=512):
#     nodes = PriorityQueue()
#     nodes.put((0, src))
...
def graph_tokenizer(adj, edge_index, node_feature, global_degree, edge_feature, src, dst,  max_nodes=512):
    ...
    z, adj, node_feature, local_degree, global_degree, node_dist, edge_index, edge_feature = get_z_restricted_graph(
        z, adj, node_feature, local_degree, global_degree, node_dist, edge_index, edge_feature, max_nodes=max_nodes)

    shortest_path_result = shortest_path(
        adj, directed=False, unweighted=True)

    # shortest_path_wo_src = np.insert(shortest_path_wo_src, src, 0, axis=0)
    # shortest_path_wo_src[0] = dist2dst
    ...
    adj = pad_till_nodes(torch.tensor(
        adj.toarray(), dtype=torch.long), max_nodes)
    spatial_pos = pad_till_nodes(spatial_pos, max_nodes)
    return z.to(torch.long), adj, node_feature, global_degree, local_degree, node_dist, edge_index, edge_feature, spatial_pos


def pad_till_nodes(x, max_nodes):
    x_new = torch.zeros((max_nodes, max_nodes), dtype=torch.long)
    x_new[:x.shape[0], :x.shape[1]] = x
    return x_new


def get_z_restricted_graph(z, adj, node_feature, global_degree, local_degree, node_dist, edge_index, edge_feature, z_limit=3, z_edge_limit=3, max_nodes=512, max_node_dist=10):
    zero_z_val = z.max()+1
    z[z == 0] = zero_z_val
    ...
    z = z[z_limit_mask]
    local_degree = local_degree[z_limit_mask]
    ...
    return z, adj, node_feature, local_degree, global_degree, node_dist, edge_index, edge_feature
...
def construct_pyg_graph(node_ids, adj, dists, node_features, y, degrees, node_label='drnl', max_nodes=512):
    ...
    if node_label.startswith('drnl'):  # DRNL
        ...
        z, adj, node_feature, local_degree, global_degree, node_dist, edge_index, edge_feature, spatial_pos = graph_tokenizer(
        ...
        return CustomData(x=node_feature,
                          ...
                          adjacency_matrix=adj.view(-1)+1,
                          spatial_pos=spatial_pos.view(-1)+1)
