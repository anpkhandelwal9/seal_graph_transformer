...
class SEALDynamicDataset(Dataset):
    ...
    def get(self, idx):
        ...
        degrees[degrees > 127] = 127
        degrees = torch.ceil(torch.log2(degrees + 1)).to(torch.long)
        # degrees = torch.floor(torch.log2(degrees + 1)).to(torch.long)
...
def run(rank, world_size, args, log_file, return_queue):
    ...
    for run in range(args.runs):
        if args.model == 'DGCNN':
            ...
            model = TokenGTModel(embedding_dim=args.embedding_dim, num_classes=1,
                                 ...
                                 performer=True, performer_nb_features=None).to(rank)
    ...
    if rank == 0:
        ...
        return_queue.put(previous_loss)
    cleanup()


def run_optimization(trial, args):
    optimize_params = {
        # embedding_dim, encoder_layers and attn heads in power of 2
        'embedding_dim': trial.suggest_int('embedding_dim', 7, 9),
        'lr': trial.suggest_float('lr', 1e-5, 1e-4),
        'num_hops': trial.suggest_int("num_hops", 2, 2),
        'encoder_layers': trial.suggest_int("encoder", 1, 1),
        'num_attention_heads': trial.suggest_int("num_attention_heads", 2, 2),
        'weight_decay': trial.suggest_float('weight_decay', 1e-4, 1e-3)
    }
    args.embedding_dim = int(2**optimize_params['embedding_dim'])
    args.lr = optimize_params['lr']
    args.num_hops = optimize_params['num_hops']
    args.encoder_layers = int(2 ** optimize_params['encoder_layers'])
    args.num_attention_heads = int(2 ** optimize_params['num_attention_heads'])
    args.weight_decay = optimize_params['weight_decay']
    args.seed = int(time.time())
    logging_file = define_logging(args)
    return_queue = mp.SimpleQueue()

    params = (world_size, args, logging_file, return_queue)

    mp.spawn(run, args=params,
             nprocs=world_size, join=True)
    min_loss = return_queue.get()
    return min_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--early_stop_epochs', type=int, default=3)
    ...
    for dataset in datasets:
        ...
        args.num_node_features = get_num_features(dataset)
        study = optuna.create_study(
            direction="minimize", sampler=optuna.samplers.TPESampler())

        def optimize_func(trial): return run_optimization(trial, args)
        study.optimize(optimize_func, n_trials=30)
