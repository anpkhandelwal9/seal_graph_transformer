...
class GraphFeatureTokenizer(nn.Module):
    ...
    def __init__(
        ...
        self.in_degree_encoder = nn.Embedding(
            25, hidden_channels, padding_idx=24)

        self.subg_degree_encoder = nn.Embedding(
            25, hidden_channels, padding_idx=24)

        self.z_encoder = nn.Embedding(
            max_z, hidden_dim - 2*hidden_channels, padding_idx=max_z-1)
    ...
    def forward(self, batched_data, perturb=None):
        ...
        if z_data.max() >= 1000 or global_degree.max() >= 20 or local_degree.max() >= 20 or node_dist.max() >= 100 or z_data.min() < 0 or local_degree.min() < 0 or node_dist.min() < 0:
            print("Issue!!!")

        z_feature = self.z_encoder(z_data)  # [sum(n_node), D]
        node_dist_feature = self.dist_encoder(node_dist).sum(-2)
        in_degree_feature = self.in_degree_encoder(global_degree)
        subg_degree_feature = self.subg_degree_encoder(local_degree)
        # node_id_feature = self.node_id_encoder(node_ids)

        if self.use_node_features:
            node_feature = torch.cat([self.node_encoder(
                node_data), z_feature, in_degree_feature, subg_degree_feature], dim=1) + node_dist_feature  # + z_feature  # + node_dist_feature
            # node_feature = self.act(self.node_encoder(node_data))
        else:
            node_feature = torch.cat(
                [z_feature, in_degree_feature, subg_degree_feature], dim=1) + node_dist_feature
        ...
        if self.use_edge_encoding_bias:
            attn_bias = torch.zeros(
                [self.num_attention_heads, padded_feature.shape[0], padded_feature.shape[1], padded_feature.shape[1]], dtype=torch.float).to(device)

            attn_bias[:, :, 0, 1] = self.graph_node_bias_encoder(
                torch.tensor(1, dtype=torch.long).to(device)).unsqueeze(1).repeat(1, padded_feature.shape[0])
            attn_bias[:, :, 0, 2] = self.graph_node_bias_encoder(
                torch.tensor(1, dtype=torch.long).to(device)).unsqueeze(1).repeat(1, padded_feature.shape[0])
            ...
            attn_bias += edge_attn_bias + spatial_pos_attn_bias
        else:
            attn_bias = None
        # [B, 2+T, D], [B, 2+T], [B, T, 2]
