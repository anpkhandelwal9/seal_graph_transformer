...
def test():
    ...
    print("Validation Loss: {}".format(total_loss/len(val_dataset)))
    total_loss = 0
    y_pred, y_true = [], []
...
if __name__ == '__main__':
    ...
    train_loader = BaseLoader(train_dataset, batch_size=args.batch_size,
                              shuffle=True, num_workers=args.num_workers, collate_fn=collator)
    val_loader = BaseLoader(val_dataset, batch_size=args.batch_size,
                            num_workers=args.num_workers, collate_fn=collator)
    test_loader = BaseLoader(test_dataset, batch_size=args.batch_size,
                             num_workers=args.num_workers, collate_fn=collator)
    ...
    for run in range(args.runs):
        if args.model == 'DGCNN':
            ...
            model = TokenGTModel(embedding_dim=256, num_classes=1,
                                 num_node_features=128, num_edge_features=2, max_nodes=512).to(device)
