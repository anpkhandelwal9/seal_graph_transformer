...
if args.save_appendix == '':
    args.save_appendix = '_' + args.model + '_' + str(args.lr) + '_' + time.strftime("%Y%m%d%H%M%S")
...
for run in range(args.runs):
    if args.model == 'DGCNN':
        ...
        model = TokenGTModel(embedding_dim=1024, num_classes=1, num_node_features=1433, num_edge_features=1, max_nodes=512).to(device)
