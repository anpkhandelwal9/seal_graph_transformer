...
def run(args, run_no):
    ...
    args.res_dir = os.path.join('script_results/{}{}_noDist'.format(args.dataset, args.save_appendix))
    ...
    train_loader = BaseLoader(train_dataset, batch_size=args.batch_size, collate_fn=collator,
                            shuffle=True, generator=generator, num_workers=args.num_workers)
    val_loader = BaseLoader(val_dataset, batch_size=args.batch_size,  collate_fn=collator,#shuffle=True,
                        num_workers=args.num_workers)
    test_loader = BaseLoader(test_dataset, batch_size=args.batch_size,  collate_fn=collator,#shuffle=True,
                            num_workers=args.num_workers)
    ...
    for run in range(args.runs):
        if args.model == 'DGCNN':
            ...
            model = TokenGTModel(embedding_dim=args.embedding_dim, num_classes=1, num_node_features=args.num_node_features, num_edge_features=1, max_nodes=512).to(device)
...
embedding_dims = [1024, 512, 256, 128] #, 512, 256, 128
lrs = [1e-4, 1e-5, 1e-6]
num_hops = [3, 2, 1]  #, 2, 1
num_nodes_per_hop = [25, 50, None] #, 50, None
num_runs = 3
