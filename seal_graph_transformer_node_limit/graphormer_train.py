...
def to_device(input_dict, device):
    ...
    for k in input_dict.keys():
        if (torch.is_tensor(input_dict[k])):
            new_dict[k] = input_dict[k].to(device)
        else:
...
def train(train_dataset, train_loader, model, optimizer, device):
    ...
    for data in pbar:
        ...
        loss = loss_fn(logits.view(-1), data.y.to(torch.float)
                       ) if args.model != 'GraphTransformer' else logits.loss
        loss.backward()
        optimizer.step()
...
@torch.no_grad()
def test(model, val_loader, val_dataset, test_loader, test_dataset, device, evaluator=None):
    model.eval()
    total_loss = 0
    y_pred, y_true = [], []
...
def run(args, run_no=1):
    ...
    args.res_dir = os.path.join(
        'results/{}{}'.format(args.dataset, args.save_appendix))
    ...
    for run in range(args.runs):
        if args.model == 'DGCNN':
            ...
            model = TokenGTModel(embedding_dim=args.embedding_dim, num_classes=1,
                                 num_node_features=args.num_node_features, num_edge_features=2,
                                 max_nodes=512, encoder_layers=args.encoder_layers, encoder_attention_heads=args.num_attention_heads,
                                 performer=True, performer_nb_features=None).to(device)
        parameters = list(model.parameters())
        if args.train_node_embedding:
            torch.nn.init.xavier_uniform_(emb.weight)
            parameters += list(emb.parameters())
        optimizer = torch.optim.Adam(
            params=parameters, lr=args.lr, weight_decay=1e-5)
        ...
        if args.only_test:
            results = test(model, val_loader, val_dataset,
                           test_loader, test_dataset, device, evaluator)
            for key, result in results.items():
