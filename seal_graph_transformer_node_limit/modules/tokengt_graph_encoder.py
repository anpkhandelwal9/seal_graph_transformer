...
from .graph_attn_bias import GraphAttnBias
...
class TokenGTGraphEncoder(nn.Module):
    def __init__(
        ...
        # self.graph_attention_bias = GraphAttnBias(
        #     num_heads=num_attention_heads,
        #     num_edges=100,
        #     n_layers=num_encoder_layers
        # )
    ...
    def forward(
        ...
        if token_embeddings is not None:
            ...
            x, padding_mask, padded_index, attn_bias = self.graph_feature(
        ...
        if attn_mask is not None:
            raise NotImplementedError
        # else:
        #     attn_mask = torch.ones(x.shape[1], x.shape[0], dtype=torch.bool)
        attn_dict = {'maps': {}, 'padded_index': padded_index}
        for i in range(len(self.layers)):
            layer = self.layers[i]
            x, attn = layer(x, self_attn_padding_mask=padding_mask,
                            self_attn_mask=attn_mask, self_attn_bias=attn_bias)
