...
class SEALDynamicDataset(Dataset):
    def __init__(self, root, data, split_edge, num_hops, percent=100, split='train',
                 ...
                 max_nodes_per_hop=None, directed=False, max_nodes=512, **kwargs):
        self.data = data
        self.num_hops = num_hops
        ...
        A_copy = self.A.copy()
        A_copy[A_copy > 0] = 1
        self.degrees = torch.tensor(A_copy.sum(1)).view(-1)
    ...
    def get(self, idx):
        ...
        y = self.labels[idx]
        tmp = k_hop_subgraph_limited(src, dst, self.num_hops, self.A, self.ratio_per_hop,
                             self.max_nodes_per_hop, node_features=self.data.x,
                             y=y, directed=self.directed, A_csc=self.A_csc, max_nodes=self.max_nodes)
        degrees = self.degrees[tmp[0]]
        if y == 1 and (self.split == 'train'): # or self.split == 'valid'):
            degrees[0] -= 1
            degrees[1] -= 1

        # degrees = (degrees/self.degree_divisor).to(torch.long)
        # for i in range(degrees.shape[0]):
        #     if degrees[i] > 10:
        #         degrees[i] = 10 + torch.floor(torch.log2(degrees[i] - 9))
        # degrees = torch.ceil(torch.log2(degrees + 1)).to(torch.long)
        # degrees = torch.ceil(std_normalize(
        #     degrees.to(torch.float), 50, 50)).to(torch.long)
        # degrees = degrees.clamp(0, 100)

        if degrees.min() < 0:
            print("degrees must")
        # assert (degrees.sum() % 2 == 0)
        data = construct_pyg_graph(
            *tmp, degrees, self.node_label, self.max_nodes)
...
def train(train_dataset, train_loader, model, optimizer, rank, max_loops=10000000):
    model.train()
...
def define_logging(args):
    ...
    with open(log_file, 'a') as f:
        ...
        f.write(f'\n\tnum_nodes_per_hop: {args.max_nodes_per_hop}\n')
    return log_file
    ...
    for run in range(args.runs):
        if args.model == 'DGCNN':
            ...
            model = TokenGTModel(embedding_dim=args.embedding_dim, num_classes=1,
                                 ...
                                 max_nodes=args.max_nodes, encoder_layers=args.encoder_layers, encoder_attention_heads=args.num_attention_heads,
                                 ).to(rank)
            #  performer=True).to(rank)
        ...
        optimizer = torch.optim.AdamW(
            params=parameters, lr=args.lr)  # , weight_decay=1e-2*sqrt(args.batch_size/(len(train_dataset)*args.epochs)))
        ...
        for epoch in range(start_epoch, start_epoch + args.epochs):
            ...
            if epoch % args.eval_steps == 0 and rank == 0:
                ...
                if epoch % args.log_steps == 0:
                    ...
                    if valid_res > previous_res:
                        previous_res = valid_res
                        same_count = 0
                    else:
                        same_count += 1
                    for other_rank in range(world_size):
...
def get_encoder_layers_by_dataset(dataset):
    if dataset == 'pubmed' or dataset.startswith('ogbl'):
        return 2
    return 4


def get_num_heads_by_dataset(dataset):
    if dataset == 'pubmed' or dataset.startswith('ogbl'):
        return 4
    return 8


def get_num_workers_by_dataset(dataset):
    if dataset == 'pubmed' or dataset.startswith('ogbl'):
        return 8
    return 8
    ...
    for l in range(0, len(num_hops)):
        for j in range(0, len(embedding_dims)):
            ...
            for i in range(0, len(datasets)):
                ...
                args.lr = get_lr_by_args(args)
                args.num_hops = num_hops[l]
                args.max_nodes_per_hop = num_nodes_per_hop[l]
                args.encoder_layers = get_encoder_layers_by_dataset(
                    args.dataset)
                args.num_attention_heads = get_num_heads_by_dataset(
                    args.dataset)
                args.num_workers = get_num_workers_by_dataset(args.dataset)
