...
from self_graph_transformer import TokenGTModel
...
def run(args, run_no):
    ...
    if args.save_appendix == '':
        args.save_appendix = '_' + args.model + '_' + str(args.lr) + '_' + '_' + str(
            args.embedding_dim) + "_" + str(args.num_hops) + '_' + time.strftime("%Y%m%d%H%M%S")
    ...
    for run in range(args.runs):
        if args.model == 'DGCNN':
            ...
            model = TokenGTModel(embedding_dim=args.embedding_dim, num_classes=1, num_node_features=args.num_node_features, num_edge_features=1,
                                 max_nodes=512, performer=True).to(device)
...
if __name__ == '__main__':
    ...
    args.positional_embedding = {'z': args.z_embedding,
                                 'dist': args.dist_embedding, 'degree': args.degree_embedding}
