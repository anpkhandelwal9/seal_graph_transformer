...
def run(rank, world_size, args, log_file, return_queue):
    ...
    for run in range(args.runs):
        ...
        optimizer = torch.optim.Adam(
            params=parameters, lr=args.lr, weight_decay=1e-3*sqrt(args.batch_size/(len(train_dataset)*args.epochs)))
...
def run_optimization(trial, args):
    optimize_params = {
        ...
        'embedding_dim': trial.suggest_int('embedding_dim', 7, 10),
        'lr': trial.suggest_float('lr', 1e-6, 1e-3),
        'num_hops': trial.suggest_int("num_hops", 1, 3),
        'encoder_layers': trial.suggest_int("encoder", 0, 3),
        'num_attention_heads': trial.suggest_int("num_attention_heads", 0, 3)
    }
