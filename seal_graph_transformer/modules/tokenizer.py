...
class GraphFeatureTokenizer(nn.Module):
    ...
    def __init__(
        ...
        N = 0
        if positional_embedding['z'] :
            N += 2 
        if positional_embedding['dist'] :
            N += 2
        if positional_embedding['degree'] :
            N += 2
        if N == 0 :
            N = 1

        self.encoder_embed_dim = hidden_dim
        hidden_channels = hidden_dim//N
        #self.node_encoder = nn.Embedding(num_node_features, hidden_dim, padding_idx=0)
        N = N//2
        self.node_encoder = nn.Linear(num_node_features, hidden_dim - N*hidden_channels)
        self.node_act = nn.ReLU(inplace=True)   
        self.node_bn = nn.BatchNorm1d(num_features=hidden_dim-N*hidden_channels)
        ...
        self.dist_encoder = nn.Embedding(max_z, hidden_channels, padding_idx=max_z-1)
        self.z_encoder = nn.Embedding(max_z, hidden_channels, padding_idx=max_z-1)
        self.degree_encoder = nn.Embedding(max_z*10, hidden_channels, padding_idx=max_z*10-1)
    ...
    def forward(self, batched_data, perturb=None):
        ...
        node_feature_list = [self.node_act(self.node_bn(self.node_encoder(node_data)))]

        if self.positional_embedding['z'] :
            z_feature = self.z_encoder(z_data) # [sum(n_node), D]
            node_feature_list.append(z_feature)
        if self.positional_embedding['dist']:
            dist_feature = self.dist_encoder(dist).sum(-2)
            node_feature_list.append(dist_feature)
        if self.positional_embedding['degree']:
            degree_feature = self.degree_encoder(degree)
            node_feature_list.append(degree_feature)

        if self.use_node_features:
            node_feature = torch.cat(node_feature_list, dim=1)# [sum(n_node), D] #, z_feature
        else:
            if self.positional_embedding['z'] :
                node_feature += z_feature
            if self.positional_embedding['dist'] :
                node_feature += dist_feature
            if self.positional_embedding['degree']:
                node_feature += degree_feature
